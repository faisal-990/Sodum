...
// { dataTypes.insert(TOKEN::Int); dataTypes.insert(TOKEN::Bool);
...
//
// if (!consumeIfMatch(TOKEN::TYPE::Identifier)) {
// LogError("parameter name", tokens[current].lexeme, tokens[current].line);
// return nullptr;
//}
...
//
// if (!match(TOKEN::TYPE::Rpar)) {
// LogError("Expected ')' after else-if condition", tokens[current].lexeme,
// tokens[current].line);
// synchronize();
// return nullptr;
//}
...
//}
//
// return std::make_unique<IfNode>(std::move(ifCondition), std::move(thenBlock),
...
// int line) {
// std::cerr << "Expected '" << expected << "' at line " << line << " but got '"
//<< got << "' instead\n";
//}
//
